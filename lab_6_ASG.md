...
6. Configure the security group that was created during the earlier step.
...
5. Set the Increase Group Size to the following attributes:
...
6. Set the Decrease Group Size to the following attributes:
    * Click on "Create a simple scaling policy"
    * Add a new alarm with the following attributes:
        * Uncheck the notification
        * Whenever Average of CPU Utilization is **<** 75% for at least 1 consecutive period of 5 minutes
...
7. Add a notification for the auto scaling group to notify your email address whenever instances are launched or terminated
8. Create the ASG
9. Ensure that your ASG deployed successfully by viewing the "Activity History" of the ASG
...
5. Navigate to the Instances page and review the "Monitoring" tab for the ASG instance
    * The spike in CPU utilization should be visible
...
10. Navigate to the "Monitoring" tab on the ASG page and switch the display to EC2
    * You should notice that the average CPU utilization has dropped below 70%
11. Review the "Activity History" for the ASG
...
Deleting an ASG will terminate instances
Need to also delete the launch config
